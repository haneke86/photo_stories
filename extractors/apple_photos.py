"""
apple_photos.py — Extract geotagged photo metadata from macOS Photos.sqlite.

Parses GPS coordinates, timestamps, and reverse geocoding data (binary plists)
from Apple's Photos database. Implements the PhotoExtractor protocol.
"""
...
from extractors.normalize import normalize_city, CITY_NAME_FIXES
...
class ApplePhotosExtractor:
    """
    Extract geotagged photo data from the macOS Photos.sqlite database.

    Implements the PhotoExtractor protocol.
    """

    source_name = "Apple Photos (macOS)"

    def __init__(self, db_path=None):
        self.db_path = db_path or DEFAULT_DB_PATH

    def is_available(self):
        """Check whether the Photos database exists and is readable."""
        return os.path.exists(self.db_path)

    def extract(self, debug_plists=False, **kwargs):
        """
        ...
        for row in rows:
            ...
            metro_city = normalize_city(country, raw_city, state, sub_admin) if country else None
        ...
        df["city"] = df["city"].replace(CITY_NAME_FIXES)
...
def _parse_reverse_location(blob) -> Tuple[Optional[str], Optional[str], Optional[str], Optional[str], Optional[str]]:
    ...
    values. City normalization happens separately in normalize_city().
    """
    if blob is None:
